<div class="modal" id="payments-edit-{{ $payment->id }}">
                ...
                <form action="{{ route('payments.update', $payment->id) }}" method="POST" autocomplete="off">
                    ...
                    <div class="row mb-3">
                        <div class="col">
                            ...
                            <select name="patient_id" class="form-control select2-show-search select2-dropdown">
                                ...
                                @foreach ($patients as $patient)
                                    <option value="{{ $patient->id }}" @selected($patient->id === $payment->patient->id)>{{ $patient->name }}</option>
                                @endforeach
                            </select>
                            @error('patient_id')
                        ...
                        <div class="col">
                            ...
                            <input type="number" step="0.05" name="amount" class="form-control" value="{{ $payment->amount }}">
                            @error('amount')
                                <span class="text-danger">{{ $message }}</span>
                            @enderror
                        </div>
                    ...
                    <div class="row mb-3">
                        <div class="col">
                            ...
                            <textarea id="AboutMe" name="notes" class="form-control" rows="3">{{ $payment->notes }}</textarea>
                            @error('notes')
                                <span class="text-danger">{{ $message }}</span>
                            @enderror
                        </div>
