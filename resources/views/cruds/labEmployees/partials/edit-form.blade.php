<form action="{{ auth()->guard('admin')->check() ? route('labEmployees.update', $labEmployee->id) : route('labEmployee.update') }}" method="POST" autocomplete="off" enctype="multipart/form-data">
    @csrf
    @method('PUT')
    ...
    <div class="row row-xs align-items-center mg-b-20">
        ...
        <div class="col-md-8 mg-t-5 mg-md-t-0">
            <select class="form-control" name="gender">
                <option value="1" @selected($labEmployee->gender)>{{ __('const.gender.male') }}</option>
                <option value="0" @selected(!$labEmployee->gender)>{{ __('const.gender.female') }}</option>
            </select>
            @error('gender')
            <span class="text-danger">{{ $message }}</span>
            @enderror
    ...
    <div class="row row-xs align-items-center mg-b-20">
        ...
        <div class="col-md-8 mg-t-5 mg-md-t-0">
        <textarea id="AboutMe" name="bio" class="form-control" rows="5">{{ $labEmployee->bio }}</textarea>
            @error('bio')
            <span class="text-danger">{{ $message }}</span>
            @enderror
        </div>
    ...
    <div class="row row-xs align-items-center mg-b-20">
        ...
        <div class="col-md-3 mg-t-5 mg-md-t-0">
            <img style="border-radius:50%" width="150px" height="150px" id="output"
                src="{{ asset('backend/images/' . $labEmployee->image->path) }}" />
        </div>
    </div>
