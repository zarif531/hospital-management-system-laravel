        ...
        <div class="main-header-profile bg-primary p-3">
            <div class="d-flex wd-100p">
                <div class="main-img-user">
                    <img alt="" src="{{ asset('backend/images/' . $labEmployee->image->path) }}" class="">
                </div>
                <div class="ml-3 my-auto">
                    <h6>{{ $labEmployee->name }}</h6>
                    <span>{{ $labEmployee->email }}</span>
                </div>
            </div>
        </div>

        <a class="dropdown-item" href="{{ route('labEmployee.show') }}">
            <i class="bx bx-user-circle"></i>{{ __('general.words.profile') }}
        </a>
        <form action="{{ route('labEmployee.logout') }}" method="POST">
            @csrf
